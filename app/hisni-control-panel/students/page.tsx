...
export default function AdminPage() {
  ...
  const [teacherForm, setTeacherForm] = useState({
    firstName: "", lastName: "", email: "", password: "", phone: "", teacherYear: "year1", teacherSubject: "",
  });
  ...
  const fetchAll = () => { fetchStudents(); fetchSessions(); fetchTeachers(); };
  ...
  const fetchStudents = async () => {
    ...
    if (Array.isArray(data)) {
      setStudents(data.filter((u: Student) => !u.isTeacher && u.email !== ADMIN_EMAIL));
    }
  ...
  const fetchTeachers = async () => {
    ...
    const data = await res.json();
    if (Array.isArray(data)) {
      setTeachers(data.filter((u: Teacher) => u.isTeacher === true));
    }
  ...
  const saveTeacher = async () => {
    if (!teacherForm.firstName || !teacherForm.lastName || !teacherForm.email) {
      setTeacherMsg("يرجى ملء الاسم والإيميل"); return;
    }
    if (!editingTeacher && !teacherForm.password) {
      setTeacherMsg("كلمة المرور مطلوبة لإنشاء حساب جديد"); return;
    }
    ...
    if (editingTeacher) {
      ...
      const body: Record<string, unknown> = {
        ...
        teacherYear: teacherForm.teacherYear,
        teacherSubject: teacherForm.teacherSubject,
        isTeacher: true,
      };
      ...
      if (!registerRes.ok) {
        setTeacherMsg(registerData?.error?.message || "حدث خطأ في إنشاء الحساب");
        setSavingTeacher(false); return;
      }
      // تحديث البيانات + تفعيل is_teacher
  ...
  const deleteTeacher = async (teacher: Teacher) => {
    ...
    const jwt = localStorage.getItem("adminJwt");
    await fetch(`${STRAPI_URL}/api/users/${teacher.id}`, {
  ...
  const startEditTeacher = (teacher: Teacher) => {
    ...
    setTeacherForm({
      ...
      email: teacher.email, password: "", phone: teacher.phone || "",
      teacherYear: teacher.teacherYear || "year1", teacherSubject: teacher.teacherSubject || "",
    });
        ...
        {activeTab === "students" && (
          <>
            <div className="flex flex-wrap gap-3 mb-5">
              ...
              <button onClick={() => exportCSV(filtered)} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-green-600 text-white hover:bg-green-700 transition">
                📥 تصدير Excel
              </button>
            </div>
            ...
            <div className="flex-1">
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => { setShowTeacherForm(true); setEditingTeacher(null); setTeacherForm({ firstName: "", lastName: "", email: "", password: "", phone: "", teacherYear: "year1", teacherSubject: "" }); }}
                  className="bg-[var(--gold)] text-white px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 transition">
                  + إضافة مدرس
                </button>
              </div>
            ...
            {showTeacherForm && (
              <div className="w-80 shrink-0">
                <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-8">
                  ...
                  <div className="flex flex-col gap-3">
                    <div>
                      <label className="block text-xs font-medium text-[var(--text-gray)] mb-1">الاسم الأول *</label>
                      <input value={teacherForm.firstName} onChange={(e) => setTeacherForm(p => ({ ...p, firstName: e.target.value }))} placeholder="الاسم الأول" className={inp} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-[var(--text-gray)] mb-1">الاسم الأخير *</label>
                      <input value={teacherForm.lastName} onChange={(e) => setTeacherForm(p => ({ ...p, lastName: e.target.value }))} placeholder="الاسم الأخير" className={inp} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-[var(--text-gray)] mb-1">البريد الإلكتروني *</label>
                      <input type="email" value={teacherForm.email} onChange={(e) => setTeacherForm(p => ({ ...p, email: e.target.value }))}
                        placeholder="teacher@example.com" className={inp} disabled={!!editingTeacher} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-[var(--text-gray)] mb-1">{editingTeacher ? "كلمة مرور جديدة (اختياري)" : "كلمة المرور *"}</label>
                      <input type="password" value={teacherForm.password} onChange={(e) => setTeacherForm(p => ({ ...p, password: e.target.value }))}
                        placeholder="6 أحرف على الأقل" className={inp} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-[var(--text-gray)] mb-1">رقم الهاتف</label>
                      <input value={teacherForm.phone} onChange={(e) => setTeacherForm(p => ({ ...p, phone: e.target.value }))} placeholder="+963..." className={inp} />
                    </div>
                    <div>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-[var(--text-gray)] mb-1">المادة التي يدرّسها</label>
                      <input value={teacherForm.teacherSubject} onChange={(e) => setTeacherForm(p => ({ ...p, teacherSubject: e.target.value }))} placeholder="مثال: أصول الفقه" className={inp} />
                    </div>
                    <div className="flex gap-2 mt-1">
                      <button onClick={saveTeacher} disabled={savingTeacher}
                        className="flex-1 bg-[var(--gold)] text-white py-2 rounded-lg font-semibold hover:opacity-90 transition disabled:opacity-60 text-sm">
                        {savingTeacher ? "جاري الحفظ..." : editingTeacher ? "حفظ" : "إنشاء الحساب"}
                      </button>
                      <button onClick={() => { setShowTeacherForm(false); setEditingTeacher(null); }}
                        className="border border-gray-200 text-[var(--text-gray)] px-4 py-2 rounded-lg text-sm hover:bg-gray-50 transition">إلغاء</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6">
                ...
                <div className="flex flex-col gap-3">
                  <div>
                    ...
                    <input value={sessionForm.title} onChange={(e) => setSessionForm(p => ({ ...p, title: e.target.value }))} placeholder="مثال: لقاء إثرائي" className={inp} />
                  </div>
                  <div>
                  ...
                  <button onClick={saveSession} disabled={savingSession} className="w-full bg-[var(--gold)] text-white py-2 rounded-lg font-semibold hover:opacity-90 transition disabled:opacity-60">
                  ...
                  {editingSession && (
                    <button onClick={() => { setEditingSession(null); setSessionForm({ title: "", date: "", zoomLink: "", academicYear: "year1", isActive: true }); }}
                      className="w-full border border-gray-200 text-[var(--text-gray)] py-2 rounded-lg font-semibold hover:bg-gray-50 transition text-sm">إلغاء التعديل</button>
                  )}
                </div>
              </div>
            </div>
