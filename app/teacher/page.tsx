...
type Tab = "students" | "sessions" | "add";
...
export default function TeacherPage() {
  ...
  useEffect(() => { if (authed && teacherInfo) { fetchStudents(); fetchSessions(); } }, [authed, teacherInfo]);
                ...
                {loading ? <div className="text-center py-10 text-[var(--gold)]">جاري التحميل...</div>
                  : filteredStudents.length === 0 ? (
                    <div className="text-center py-10">
                      ...
                      <p className="text-[var(--text-gray)]">لا يوجد طلاب في سنتك</p>
                    </div>
                  ) : (
                ...
                <div className="w-64 shrink-0">
                  <div className="bg-white rounded-2xl shadow-sm p-5 sticky top-6 border border-gray-100">
                    ...
                    <div className="flex flex-col gap-1.5 text-xs">
                      ...
                      <InfoRow label="السنة" value={yearMap[selectedStudent.academicYear || ""] || "غير محدد"} />
            ...
            <div>
              {sessions.length === 0 ? (
                <div className="text-center py-16 bg-white rounded-2xl">
                  ...
                  <p className="text-[var(--text-gray)] text-sm mb-4">أضف جلستك الأولى الآن</p>
                  <button onClick={() => setActiveTab("add")} className="bg-[var(--gold)] text-black px-6 py-2 rounded-xl font-bold hover:opacity-90 transition text-sm">إضافة جلسة</button>
                </div>
              ) : (
                      ...
                      <div key={session.id} className={`bg-white rounded-xl p-4 shadow-sm border ${isPast ? "border-gray-100 opacity-60" : "border-[var(--gold)]/20"}`}>
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1">
                            ...
                            {!isPast && <a href={session.zoomLink} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 hover:underline mt-1 block truncate max-w-xs">{session.zoomLink}</a>}
                          </div>
                          <div className="flex flex-col gap-1.5 shrink-0">
                            <button onClick={() => { setEditingSession(session); setSessionForm({ title: session.title || "", date: session.date?.slice(0, 16) || "", zoomLink: session.zoomLink || "", academicYear: session.academicYear || "year1", isActive: session.isActive ?? true }); setActiveTab("add"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <div className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-right text-[var(--text-gray)] bg-gray-50 text-sm">{yearMap[teacherYear]}</div>
                  </div>
                  <div>
                  ...
                  <div className="flex gap-3">
                    <button onClick={saveSession} disabled={saving} className="flex-1 bg-[var(--gold)] text-black py-3 rounded-xl font-bold hover:opacity-90 transition disabled:opacity-60">
                    ...
                    {editingSession && (
                      <button onClick={() => { setEditingSession(null); setSessionForm({ title: "", date: "", zoomLink: "", academicYear: teacherYear, isActive: true }); setActiveTab("sessions"); }}
                        className="border border-gray-200 text-[var(--text-gray)] px-6 py-3 rounded-xl hover:bg-gray-50 transition text-sm">إلغاء</button>
                    )}
                  </div>
                </div>
              </div>
